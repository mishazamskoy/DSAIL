-- Rewrite get_leaderboard() to derive metrics from the actual city state
-- (city_structures + city_territory) instead of the user_data wellness-point columns.
--
-- Formulas mirror City.jsx:
--   grain      = COUNT(granary) * 10
--   population = 20 + COUNT(house) * 4
--   happiness  = 10 - FLOOR(population / 4) - GREATEST(population - grain, 0)
--   money      = 20 + COUNT(market) * 10 - COUNT(territory cells)
...
as $$
  with city_metrics as (
    select
      ud.user_id,
      coalesce(s.num_granaries, 0) * 10                                                   as grain,
      20 + coalesce(s.num_houses, 0) * 4                                                  as population,
      20 + coalesce(s.num_markets, 0) * 10 - coalesce(t.territory_count, 0)              as money,
      coalesce(s.num_granaries, 0) * 10                                                   as _grain_raw,
      20 + coalesce(s.num_houses, 0) * 4                                                  as _pop_raw
    from user_data ud
    left join (
      select
        user_id,
  ...
  city_with_happiness as (
    select
      ...
      population::integer,
      money::integer,
      (10 - floor(population / 4.0) - greatest(population - grain, 0))::integer as happiness
    from city_metrics
  ),
  city_with_power as (
    select
